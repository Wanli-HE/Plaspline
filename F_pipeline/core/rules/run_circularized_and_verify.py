...
usetoola = config['circularized_tool']


if usetoola == "metaplasmidspades":
    rule metaplasmidSPAdes:
        input:
            f1 = "qc_reads/{sample}_qc_1.fastq.gz",
            f2 = "qc_reads/{sample}_qc_2.fastq.gz"
        output:
            f = temp(directory("{sample}_metaplasmidspades"))
...
elif usetoola == "recycler":
    rule make_bam_file_for_run_recycler:
        input:
            f1="assmebly_res/{sample}_contigs.fasta",
            forward = "qc_reads/{sample}_qc_1.fastq.gz",
            reverse = "qc_reads/{sample}_qc_2.fastq.gz"
        output:
            f = temp("circular/{sample}_reads_pe.bam")
        threads: config["threads"]
        conda:
            "%s/circularized-recycler.yaml" % CONDAENV
        log:
            err = "log/make_bam_file_for_run_recycler/{sample}.err",
            out = "log/make_bam_file_for_run_recycler/{sample}.out"
        script:
            "../scripts/recycler_bam.py"
    ...
    rule recycler:
        input:
            ...
            f2="circular/{sample}_reads_pe_primary.sort.bam"
        output:
            f1=temp(directory("circular/{sample}_circular_out"))
        threads: config['threads']
        conda:
            "%s/circularized-recycler.yaml" % CONDAENV
        params:
            k = config['recycler_params_k'],
            rp = config['recycler_path']
        log:
            out = os.path.join("log", "recycler", "{sample}_plasmid.out"),
            err = os.path.join("log", "recycler", "{sample}_plasmid.err")
        shell:
            "samtools index {input.f2};" \
            "python {config[recycler_path]}  -g {input.f1} " \
                                            "-k {params.k} " \
                                            "-b {input.f2} " \
                                            "-o {output.f1} " \
                                            "2>{log.err} >{log.out};" \
    ...
    rule clean_recycler_res:
        input:
            f="circular/{sample}_circular_out"
        output:
            f="circular/{sample}_cycs.fasta"
        threads: config['threads']
        run:
            if os.path.exists(input.f):
                folder_name=os.path.basename(input.f)[:-len("_recycler_out")]
                file_cir=folder_name+"_contigs.cycs.fasta"
                os.system("cp "+input.f+"/"+file_cir+" "+output.f)
                os.system("rm -rf "+input.f)
            else:
                print(f"{input.f} is not exist, don`t have recycler res !!!!!!!!!!!!")
    ...
    rule scapp:
        ...
        shell:
            "python {config[scapp_path]} -g {input.f1} " \
                    "-o {output.f1} " \
                    "-k {params.k} " \
                    "-r1 {input.f2} " \
                    "-r2 {input.f3} " \

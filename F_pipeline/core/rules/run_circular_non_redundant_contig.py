...
rule linear_classify_platon:
    input:
        f = "circular_non_readundant_contig/circular_non_redundant_contigs.fa"
    output:
        f ="circular_non_readundant_contig/clasify_platon"
    ...
    params:
        ...
        db = config["platondb"]
    log:
        out = "log/classify-circular-contig/platon_.out",
        err = "log/classify-circular-contig/platon_.err"
    shell:
        "platon {input.f} " \
                "--db {params.db} " \
                "--mode {params.mode} " \
...
rule circular_makefaa:
    ...
    shell:
        "prodigal -p {params.p} -i {input.f} -a {output.f1}  2>{log.err} >{log.out}"
...
rule circular_contig_annotation_plsdb:
    input:
        f = "circular_non_readundant_contig/circular_non_redundant_contigs.fa"
    output:
        f = "circular_non_readundant_contig/co-exist/annotation_plsdb/annotation_plsdb.txt"
    threads: config["threads"]
    conda:
        "%s/non_redundant.yaml" % CONDAENV
    params:
    ...
    log:
        out = "log/co-exist/circular_plsdb.out",
        err = "log/co-exist/circular_plsdb.err"
    shell:
        "diamond {params.md} " \
                "--db {params.db} " \
                "--query {input.f} " \

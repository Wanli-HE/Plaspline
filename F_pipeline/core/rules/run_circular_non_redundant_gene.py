...
rule circular_reads_mapping_gene:
    input:
        f="circular_non_readundant_gene/{sample}_plasmid_sort.bam"
    output:
        f="circular_non_readundant_gene/mapping/{sample}_bp_mapping.txt"
    threads: config['threads']
    conda:
...
rule filter_dectect_circular_gene:
    input:
        f = "circular_non_readundant_gene/mapping/{sample}_bp_mapping.txt",
        f2 = "circular_non_readundant_gene/circular_non_redundant_genes.fa"
    output:
        f = temp("circular_non_readundant_gene/{sample}_gene_coverage.txt")
    run:
        dict_len={}
...
rule circularr_relative_gene_abundance_gene:
    input:
        f = "circular_non_readundant_gene/{sample}_gene_coverage.txt"
    output:
        f = temp("circular_non_readundant_gene/{sample}_relative_abundance.txt")
    run:
        a = 0
        with open(input.f,"r") as infile:
            for line in infile:
...
rule circular_paste_relative_gene_abundance_gene:
    input:
        f = expand("circular_non_readundant_gene/{sample}_relative_abundance.txt",sample=config["samples"])
    output:
